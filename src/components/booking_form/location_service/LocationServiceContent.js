...
import { fetchBiz, fetchLocationServices } from '../../../actions/index';
...
class LocationServiceContent extends React.Component {
  ...
  onChangeLocation = (newValue, newValueIndex) => {
    this.setState({
      location: this.props.business.locations[newValueIndex],
      currentStep: 1
    })
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.business.locations && nextProps.business.locations.length === 1){
      this.setState({
          location: this.props.business.locations[0],
          currentStep: 1
      })
    }
  }
  ...
  componentWillUpdate(nextProps, nextState) {
    if (nextState.currentStep == 1 ){
      const locationId = this.state.location.id;
      ...
      <div className="LocationContainer">
        <Stepper>
          <Step 
            style={{height: "55px"}}
            stepLine 
            completed={this.state.location.address !== undefined} 
...
export default connect(
  mapStateToProps,
  { fetchBiz, fetchLocationServices }
